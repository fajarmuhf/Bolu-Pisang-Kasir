...
?>
		<form action="?page=barang&i=edit&kirim=1" method="post" enctype="multipart/form-data">
			...
			<tr>
				<td>Barcode : </td><td><input type="text" name="barcode" id="barcode"></td>
			</tr>
			<tr>
				...
				if(isset($_GET['id'])){
					...
					if($exkueh){
						while($hasil = mysqli_fetch_array($exkueh)){
							...
							echo "document.getElementById('barcode').value='".$hasil['barcode']."';";
							echo "</script>";
						}
					}
				}
									...
									$mobilbaru->setHarga($harga);
									 ...
									 else
									 {
									 	if(isset($_FILES["gambar"]) && @$_FILES["gambar"]["size"] > 0){
											...
											$query = "UPDATE `produk` SET nama = ?,deskripsi = ?,stock = ?,satuan = ?,modal = ?,harga = ?,perum = ?,tag = ?,jumlahklik = ?,expdate = ?,barcode = ? WHERE id = ? ";
											$exquery=$Koneksi->getKonek()->prepare($query);
											$exquery->bind_param("ssisiississi",$nama,$deskripsi,$stok,$satuan,$modal,$harga,$perum,$tag,$jumlahklik,$expire,$barcode,$nilai);
										}
