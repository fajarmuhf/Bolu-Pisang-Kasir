		...
		?>
		<form action="?page=perumahan&i=delete&kirim=1" method="post">
			...
			<?php
				...
				if(@$_GET["kirim"] == 1){
					if(@$_POST["atribut"] != "" && @$_POST["nilai"] != ""){
						...
						$kueh = "SELECT *,COUNT(*) FROM `perumahan` WHERE id = ?";
							...
							if(true){
								...
								if($exquery){
									...
									if($exquery2){
										...
										if($hitung[0]['COUNT(*)'] > 0){
											$query3 = "UPDATE `perumahan` SET id = (id-1) WHERE id > ?";
