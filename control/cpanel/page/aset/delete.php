...
?>
		<form action="?page=aset&i=delete&kirim=1" method="post">
			...
			<?php
				...
				if(@$_GET["kirim"] == 1){
					if(@$_POST["atribut"] != "" && @$_POST["nilai"] != ""){
						...
						$kueh = "SELECT *,COUNT(*) FROM aset WHERE id = ? AND id_pemilik = ?";
						$exquery=$Koneksi->getKonek()->prepare($kueh);
						$exquery->bind_param("ii",$nilai,$id_pemilik);
						$exkueh = $exquery->execute();
						if($exkueh){
							$hasil = $exquery->get_result()->fetch_all(MYSQLI_ASSOC);
							if(true){
								$query = "DELETE FROM `aset` WHERE id = ? AND id_pemilik = ?";
								$exquery21=$Koneksi->getKonek()->prepare($query);
								$exquery21->bind_param("ii",$nilai,$id_pemilik);
								$exquery = $exquery21->execute();
								if($exquery){
									unlink("../../../images/".$hasil[0]['gambar']);
									$query2 = "SELECT COUNT(*) FROM aset WHERE id > ?";
									...
									if($exquery2){
										...
										if($hitung[0]['COUNT(*)'] > 0){
											$query3 = "UPDATE aset SET id = (id-1) WHERE id > ?";
