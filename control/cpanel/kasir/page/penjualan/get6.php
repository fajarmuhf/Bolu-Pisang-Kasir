<?php
	...
	if(isset($_POST['start']) && @$_POST['start'] >= 0){
		...
		$start = $_POST['start'];		
		$query = "SELECT * FROM `keluar` WHERE usermanagerid = $iduser AND perum = '$perum' AND tanggal = '$tglakhir' limit ?,$banyakindex ";
		$exquery=$Koneksi->getKonek()->prepare($query);
				...
				if($exquery2){
					...
					$html .= "<tr>
						<td style=\"font-size: large; font-weight: bold;width: 60%;\">
							".$hasil2['nama']."<br>
							<div style='color:".($hasil[$i]['status'] == 'kirim' ? 'green' : 'red')."'>".$hasil[$i]['status']."</div><br>
