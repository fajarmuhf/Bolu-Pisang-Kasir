			<?php
				...
				if(@$_POST['Aksi'] != "" && @$_POST['identitas'] != "" && @$_POST['banyakitem'] != ""){
					if($_POST['Aksi'] == 'retur'){
						...
						$query2 = "SELECT id,COUNT(*) FROM `keluar` WHERE itemid = $itemid AND usermanagerid = $iduser AND tanggal = '$tanggal' AND status = 'retur' ";
						$exquery2 = mysqli_query($Koneksi->getKonek(),$query2);
						if($exquery2){
							$hasil2 = mysqli_fetch_array($exquery2);
							if($hasil2["COUNT(*)"] > 0){
								$idcart = $hasil2["id"];
								$query = "UPDATE `keluar` SET jumlah = jumlah + ? WHERE id = ? AND status = 'retur' ";
								$exquery=$Koneksi->getKonek()->prepare($query);
								...
								$query = "INSERT INTO `keluar` SELECT (COUNT(*)+1),?,?,?,?,?,'retur' FROM `keluar` WHERE 1 ";
								$exquery=$Koneksi->getKonek()->prepare($query);
						...
						$query2 = "SELECT id,COUNT(*) FROM `keluar` WHERE itemid = $itemid AND usermanagerid = $iduser AND tanggal = '$tanggal' AND status = 'kirim' ";
						$exquery2 = mysqli_query($Koneksi->getKonek(),$query2);
						if($exquery2){
							$hasil2 = mysqli_fetch_array($exquery2);
							if($hasil2["COUNT(*)"] > 0){
								$idcart = $hasil2["id"];
								$query = "UPDATE `keluar` SET jumlah = jumlah + ? WHERE id = ? AND status = 'kirim' ";
								$exquery=$Koneksi->getKonek()->prepare($query);
								$exquery->bind_param("ii",$banyakitem,$idcart);
								$result = $exquery->execute();
								if($result){
								...
								$query = "INSERT INTO `keluar` SELECT (COUNT(*)+1),?,?,?,?,?,'kirim' FROM `keluar` WHERE 1 ";
								$exquery=$Koneksi->getKonek()->prepare($query);
								$exquery->bind_param("iiiss",$iduser,$itemid,$banyakitem,$perum,$tanggal);
								$result = $exquery->execute();
								if($result){
						...
						$query2 = "SELECT id,COUNT(*) FROM `keluar` WHERE itemid = $itemid AND usermanagerid = $iduser AND tanggal = '$tanggal' AND status = ? ";
						$exquery2=$Koneksi->getKonek()->prepare($query2);
						$exquery2->bind_param("s",$status);
						$exquery2->execute();
						if(true){
							$hasil2 = $exquery2->get_result()->fetch_all(MYSQLI_ASSOC);
							if($hasil2[0]["COUNT(*)"] > 0){
								$idcart = $hasil2[0]["id"];
								$query = "UPDATE `keluar` SET jumlah = jumlah + ? WHERE id = ? AND status = ? ";
								$exquery=$Koneksi->getKonek()->prepare($query);
								$exquery->bind_param("iis",$banyakitem,$idcart,$status);
								$result = $exquery->execute();
								if($result){
									$n = $itemid;
									$j = $banyakitem;
								...
								$query = "INSERT INTO `keluar` SELECT (COUNT(*)+1),?,?,?,?,?,? FROM `keluar` WHERE 1 ";
								$exquery=$Koneksi->getKonek()->prepare($query);
								$exquery->bind_param("iiisss",$iduser,$itemid,$banyakitem,$perum,$tanggal,$status);
								$result = $exquery->execute();
								if($result){
									$n = $itemid;
									$j = $banyakitem;
					...
					$namaperumahan = 'retur';
					$_SESSION['status'] = $namaperumahan;	
					$qrshowstatus = 'false';
					$_SESSION['qrshowstatus'] = $qrshowstatus;	
				}				
								
				if(@$_POST['status'] != ""){
					$namaperumahan = $_POST['status'];
					$_SESSION['status'] = $namaperumahan;
				}
				else{
				...
				if($exquery){
					...
					echo "<h1 class='title'>Retur/Kirim Barang</h1>";
					...
					echo "</ul>";
					echo "<select name='status' id='status' style='margin:auto;width:50%;margin-bottom:15px;border-radius:25px;' >";

					$perumold = $namaperumahan;

					if(strcmp($perumold, "kirim") == 0){
						echo '<option value="retur" selected>retur</option>';
						echo '<option value="'.$perumold.'" selected>'.$perumold.'</option>';
					}
					else{
						echo '<option value="'.$perumold.'">'.$perumold.'</option>';
						echo '<option value="kirim">kirim</option>';
					}
			...
			<script>
				...
				function loadMore(start,key,minharga,maxharga){
					if(lagiloading==false){
						lagiloading=true;
					jQuery.ajax({
						url:'kasir/page/<?php echo $namapage;?>/get7.php',
