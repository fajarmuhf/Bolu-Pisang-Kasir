	...
	if(isset($_POST['start']) && @$_POST['start'] >= 0){		
		...
		if(isset($_POST['key']) && @$_POST['key'] != ""){
			...
			if(isset($_POST['minharga']) && $_POST['minharga'] != ""){
				...
				if(isset($_POST['maxharga']) && $_POST['maxharga'] != ""){
					...
					$query = "SELECT * FROM `produk` WHERE harga <= ? AND perum = ? limit ?,$banyakindex ";
					$exquery=$Koneksi->getKonek()->prepare($query);
					$exquery->bind_param("ii",$maxharga,$perum,$start);
				}
				else{
		...
		if($result){
			...
			for($i=0;$i<count($hasil);$i++){
				$html .= "<tr>
						<td style=\"font-size: large; font-weight: bold;\" onclick='document.getElementById(\"Aksi".$hasil[$i]['id']."\").value=\"tambah\";document.getElementById(\"Aksi".$hasil[$i]['id']."\").name=\"Aksi\";document.getElementById(\"identitas\").value=\"".$hasil[$i]['id']."\";document.getElementById(\"banyakitem\").value=document.getElementById(\"banyak".$hasil[$i]['id']."\").value;document.getElementById(\"daftar\").submit();'>
						<input type=hidden id='banyak".$hasil[$i]['id']."' name='banyak".$hasil[$i]['id']."' placeholder='banyak item..' value=1>
						<input type=hidden id='Aksi".$hasil[$i]['id']."' name='Aksi".$hasil[$i]['id']."'>
						".$hasil[$i]['nama']."<br>
						<a href='../images/".$hasil[$i]['imageurl']."'  onclick='window.event.stopPropagation();return hs.expand(this)' class='highslide'><img src='../images/".$hasil[$i]['imageurl']."' width=120px height=120px ></a><br>
