		<form action="?page=mentah&i=input&kirim=1" method="post" enctype="multipart/form-data">
			...
			<?php
				...
				if(@$_GET["kirim"] == 1){
					if(@$_POST["nama"] != "" && @$_POST["harga"] != "" && @$_POST["ket"] != "" && @$_FILES["gambar"]["error"] <= 0){
						...
						$kueh = "SELECT (COUNT(*)+1) FROM `Barang_Mentah` WHERE 1 ";
						...
						if($exkueh){
							...
							$barangbaru->setGambar($gambar);
							 ...
							 else
							 {
								...
								$query = "INSERT INTO `Barang_Mentah` SELECT (COUNT(*)+1),'".$barangbaru->getNama()."','".$barangbaru->getHarga()."','".$barangbaru->getKet()."','".$barangbaru->getGambar()."' FROM `Barang_Mentah` WHERE 1 ";
