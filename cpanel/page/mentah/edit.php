		<form action="?page=mentah&i=edit&kirim=1" method="post" enctype="multipart/form-data">
					...
					@$_POST["atribut"] != "" && @$_POST["nilai"] != ""){
						...
						if($exkueh){
							while($hasil = mysql_fetch_array($exkueh)){
								...
								$kueh = "SELECT Id FROM `Barang_Mentah` WHERE $atribut = $nilai ";
								$exkueh = mysql_query($kueh);
								if($exkueh){
									...
									$mobilbaru->setGambar($gambar);
									 ...
									 else
									 {
										...
										$query = "UPDATE `Barang_Mentah` SET Nama = '".$mobilbaru->getNama()."',Harga = '".$mobilbaru->getHarga()."',Keterangan = '".$mobilbaru->getKet()."',Gambar = '".$mobilbaru->getGambar()."' WHERE $atribut = '$nilai' ";
