		<form action="?page=penjualan&i=restock&kirim=1" method="post">
			...
			<?php
				...
				if(@$_GET["id"] != ""){
						...
						$kueh = "SELECT * FROM clientorder as A,cart as B WHERE A.restock IS NULL AND A.status = 'Dibatalkan' AND A.id = ? AND A.id = B.orderid";
						...
						if($exkueh){
							...
							$query = "UPDATE clientorder SET restock = 'sudah' WHERE id = ? ";
							...
							for($i=0;$i<count($hasil);$i++){
								...
								if($exquery){
									...
									echo "Anda tidak berhasil merestock data<br>";
								}
							}
							if(count($hasil)==0){
								echo "Anda tidak berhasil merestock data<br>";
