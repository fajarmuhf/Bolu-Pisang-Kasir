		<form action="?page=rekening&i=delete&kirim=1" method="post">
			...
			<tr>
				...
				<td>
					<select name="atribut" id="atribut">
						...
						<option value="Id_Rekening">Id Rekening</option>
			...
			<?php
				...
				if(@$_GET["kirim"] == 1){
					if(@$_POST["atribut"] != "" && @$_POST["nilai"] != ""){
						...
						$query = "SELECT COUNT(*) FROM Rekening WHERE $atribut = '$nilai' ";
						$exquery = mysql_query($query);
						if($exquery){
							$hasil = mysql_fetch_array($exquery);
							if($hasil[0] > 0){
								$query2 = "SELECT * FROM Rekening WHERE $atribut = '$nilai'";
								$exquery2 = mysql_query($query2);
								if($exquery2){
									$kueh = "DELETE FROM `Rekening` WHERE $atribut = '$nilai' ";
									$exkueh = mysql_query($kueh);
									if($exkueh){
										while($hasil2 = mysql_fetch_array($exquery2)){
											$query3 = "SELECT COUNT(*) FROM Rekening WHERE Id > ".$hasil2['Id'];
											$exquery3 = mysql_query($query3);
											if($exquery3){
												$hitung = mysql_fetch_array($exquery3);
												if($hitung[0] > 0){
													$query4 = "UPDATE Rekening SET Id = (Id-1) WHERE Id > ".$hasil2['Id'];
